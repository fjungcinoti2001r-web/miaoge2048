    ...
    if (tile) {
      if (isNew) {
        (function () {
          ...
          var avatarNo = Math.min(Math.log2(tile.value), 11);
          tileDiv.style.backgroundImage = "url('assets/tiles/avatar_" + avatarNo + ".png')";
...
document.getElementById('enter-game-btn').addEventListener('click', function () {
  document.getElementById('intro-mask').classList.add('hidden');
  bgm.play().catch(function () {});
});
document.getElementById('bgm-toggle').addEventListener('click', function () {
  if (bgm.paused) { bgm.play().catch(function () {}); this.textContent = '🎵'; }
  else { bgm.pause(); this.textContent = '🔇'; }
});
document.getElementById('settings-toggle').addEventListener('click', function () {
  document.getElementById('settings-modal').classList.add('active');
});
document.getElementById('poster-toggle').addEventListener('click', function () {
  var high = Math.max.apply(null, game.filter(Boolean).map(function (t) { return t.value; }).concat([2]));
  document.getElementById('poster-score').textContent = score;
  document.getElementById('poster-best').textContent = bestScore;
  document.getElementById('poster-role-text').textContent = '已达成：' + high;
  document.getElementById('poster-avatar-preview').style.backgroundImage = "url('assets/tiles/avatar_" + Math.min(Math.log2(high), 11) + ".png')";
  document.getElementById('poster-modal').classList.add('active');
});
document.getElementById('poster-close').addEventListener('click', function () { document.getElementById('poster-modal').classList.remove('active'); });
document.getElementById('save-game').addEventListener('click', function () {
  localStorage.setItem('miaoge2048-save', JSON.stringify({ game: game, score: score, nextId: nextId }));
  document.getElementById('save-hint').textContent = '存档已保存';
});
document.getElementById('load-game').addEventListener('click', function () {
  var saved = localStorage.getItem('miaoge2048-save');
  if (!saved) { document.getElementById('save-hint').textContent = '暂无存档'; return; }
  saved = JSON.parse(saved); game = saved.game; score = saved.score; nextId = saved.nextId;
  scoreDiv.innerHTML = score; document.getElementById('tile-container').innerHTML = ''; drawBackground(); updateDOM([], game);
  document.getElementById('save-hint').textContent = '存档已读取'; document.getElementById('settings-modal').classList.remove('active');
});
